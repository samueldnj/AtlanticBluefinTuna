...
Explot_SS_multi<-function(MSEobj1,MSEobj2,sim=1,plotnam=NA,MPnams=c("ZeroC","CurC100"),bystock=T,S_A=2,legendy=F,MSEnams=c("","")){
  ...
  ttplot<-function(yrs,x1,x2,cols,ltys,verts=c(1965,2016,2020)){
    ...
    big<-1E10
    polygon(c(verts[2],verts[2],verts[3],verts[3]),c(-big,big,big,-big),col=filly,border=filly)
    matplot(yrs,x1,type='l',xlab="",ylab="",col=cols[1],lty=c(1,3),lwd=2,add=T)
    matplot(yrs,x2,type='l',xlab="",ylab="",col=cols[2],lty=c(1,3),lwd=2,add=T)
  }
  cols<-c("#ff000090","#0000ff90")
  ltys=rep(1,2)
  ...
  for(p in MSEobj@npop:1){
    ...
    if(p==1)legend('bottomleft',legend=MPnams,lty=c(1,3),text.font=2,bty='n',cex=1.2)
