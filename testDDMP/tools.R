...
getIndices <- function( minT=1952, maxT=2018, useGear=NULL )
{
  ...
  grs <- indexTable$parameter

  fi   <- read.csv("Data/FI_indices_compiled_OM2019.csv") %>%
          dplyr::select(Year,area,index,CV,Name)
  cpue <- read.csv("Data/CPUE_indices_compiled_2019OM.csv") %>%
          dplyr::select(Year,area=Area,index=Index,CV,Name)

  ind <- rbind(fi,cpue)

  # Gears/years
  #grs <- levels(unique(ind$Name))
  yrs <- min(ind$Year):max(ind$Year)
  nG <- length(grs)
  nT <- length(yrs)

  I_gt <- matrix( data=NA, nrow=nG, ncol=nT,
                  dimnames=list(grs,yrs) )
  area_g <- numeric(nG)
  idxType_g <- rep(3,nG)
  idxType_g[which(grs=="MED_LAR_SUV")] <- 1
  idxType_g[which(grs=="GOM_LAR_SUV")] <- 2

  #par(mfrow=c(5,5),mar=c(0,0,0,0))

  for( g in 1:nG )
  {
    gind <- filter(ind,Name==grs[g])
    I_gt[g,as.character(gind$Year)] <- gind$index
    a <- gind$area[1]
    area_g[g] <- ifelse(a>3,0,1)

#    plot(x=yrs,y=I_gt[g,],axes=FALSE)
#    box()
#    legend("topleft",grs[g],bty="n")
  }

  # Trim years and gears
  if(is.null(useGear))
    useGear <- rep(1,nG)
  ys <- which(yrs%in%c(minT,maxT))
  I_gt <- I_gt[as.logical(useGear),ys[1]:ys[2]]
  area_g <- area_g[as.logical(useGear)]
  idxType_g <- idxType_g[as.logical(useGear)]

  list( I_gt=I_gt, area_g=area_g, idxType_g=idxType_g )
